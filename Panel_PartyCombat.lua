...
function FromClient_MatchPvpResult( isWin )
	...
	for countidx = 0, resultCount-1 do
		...
		if not resultEmpty then
			...
			matchResult._Name[tempcountidx]			:SetShow( true )
			matchResult._Kill[tempcountidx]			:SetShow( true )
			matchResult._Point[tempcountidx]		:SetShow( true )
			matchResult._Icon_Kill[tempcountidx]	:SetShow( true )
			matchResult._Icon_Point[tempcountidx]	:SetShow( true )
			matchResult._Icon_Class[tempcountidx]	:SetShow( true )
		else
...
function HandleClicked_MatchOpen( compactInfo )									-- 결투정보창 열기 (ture = 작은 위젯)
	...
	if compactInfo then	-- 미니정보창
		...
		matchInfo._State_BG				:SetShow( true )
		matchInfo._State				:SetShow( true )
		...
		matchInfo._RegStatus_Text		:SetShow( true )
		matchInfo._BG					:SetShow( true )
		matchInfo._Party_Icon			:SetShow( true )
		matchInfo._Party_Text			:SetShow( true )
		matchInfo._Party_Value			:SetShow( true )
		matchInfo._Time_Icon			:SetShow( true )
		matchInfo._RemainTime_Text		:SetShow( true )
		matchInfo._RemainTime_Value		:SetShow( true )
		matchInfo._Button_Combat		:SetShow( isLeader )	-- 파티장일 경우 신청버튼 보이기
