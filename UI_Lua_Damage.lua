...
function FromClient_TendencyChanged( actorKeyRaw, tendencyValue )
	...
	if ( selfActorKeyRaw == actorKeyRaw ) and ( ( tendencyValue < 0 ) or ( 100 <= tendencyValue ) ) then
		...
		DamageOutputFunction_OnDamage( getSelfPlayer():getActorKey(), tendencyValue, 96, 10, selfPosition, false, true)
	end
end
...
function FromClient_VaryIntimacy( actorKeyRaw, intimacyValue )
	...
	if ( 0 ~= intimacyValue ) then
		...
		DamageOutputFunction_OnDamage( actorKeyRaw, intimacyValue, 97, 10, actorPosition, false, true)
	end
end
...
function DamageOutputFunction_gainExperience( experience )
	...
	if ( 0 < experience ) then
		DamageOutputFunction_OnDamage( getSelfPlayer():getActorKey(), experience, 99, 10, selfPosition, false, false)
	end
end
...
function FromClient_WpChanged(prevWp, wp)
	...
	if ( varyWp < 0 ) or ( 1 < varyWp ) then
		DamageOutputFunction_OnDamage( getSelfPlayer():getActorKey(), varyWp, 93, 10, selfPosition, false, false)
	end
end

function DamageOutputFunction_OnDamage( attakeeKeyRaw, effectNumber, effectType, additionalDamageType, posFloat3, isTribeAttack, isNotRandom )
