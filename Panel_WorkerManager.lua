...
local workerManager_Initiallize = function()
	...
	for slotIdx = 0, self.slotMaxCount -1 do
		...
		tempSlot.workerCheck	:SetPosY( 3 )
		tempSlot.workerName		:SetPosX( 30 )
		tempSlot.workerName		:SetPosY( 3 )
		tempSlot.workerRestorePT:SetPosX( 28 )
		tempSlot.workerRestorePT:SetPosY( tempSlot.workerName:GetSizeY() + 4 )
		tempSlot.workerCurrentPT:SetPosX( 28 )
		tempSlot.workerCurrentPT:SetPosY( tempSlot.workerName:GetSizeY() + 4 )

		tempSlot.progressBg		:SetPosX( 28 )
		tempSlot.progressBg		:SetPosY( tempSlot.workerRestorePT:GetPosY() + tempSlot.workerRestorePT:GetSizeY()+ 4 )
		tempSlot.progress		:SetPosX( 28 )
		tempSlot.progress		:SetPosY( tempSlot.workerRestorePT:GetPosY() + tempSlot.workerRestorePT:GetSizeY()+ 5 )
		tempSlot.workingName	:SetPosX( 32 )
		tempSlot.workingName	:SetPosY( tempSlot.progressBg:GetPosY() - 2 )

		tempSlot.btn_Restore	:SetPosX( tempSlot.bg:GetSizeX() - tempSlot.btn_Restore:GetSizeX() - 2 )
		tempSlot.btn_Restore	:SetPosY( tempSlot.workerName:GetPosY() )
		tempSlot.btn_Upgrade	:SetPosX( tempSlot.btn_Restore:GetPosX() - tempSlot.btn_Upgrade:GetSizeX() )
		tempSlot.btn_Upgrade	:SetPosY( tempSlot.btn_Restore:GetPosY() + 1 )
		tempSlot.btn_ChangeSkill:SetPosX( tempSlot.btn_Restore:GetPosX() - tempSlot.btn_ChangeSkill:GetSizeX() )
		tempSlot.btn_ChangeSkill:SetPosY( tempSlot.btn_Restore:GetPosY() + 1 )
		tempSlot.btn_Stop		:SetPosX( tempSlot.bg:GetSizeX() - tempSlot.btn_Restore:GetSizeX() - 2 )
		tempSlot.btn_Stop		:SetPosY( tempSlot.progressBg:GetPosY() - 2 )

		tempSlot.btn_Restore	:SetFontColor( UI_Color.C_FFEDE699 )
...
local workerManager_UpdateMain = function()
	...
	for worker_Index = self._startIndex, #filteredArray do
		...
		slot.btn_Stop		:SetFontColor( 4292753664 )
		...
		local setWorker = function()
			...
			if 0 < workerUpgradeCount and workerWrapperLua:isUpgradable() then
				worker_Name			= PAGetStringParam2( Defines.StringSheet_GAME, "LUA_WORKERMANAGER_UPGRADECOUNT", "name", workerWrapperLua:getName(), "count", workerWrapperLua:getUpgradePoint() )
				slot.btn_Upgrade	:SetShow( true )
				slot.btn_Upgrade	:SetFontColor( UI_Color.C_FFEFEFEF )
			else
			...
			slot.btn_Stop		:SetText( PAGetString(Defines.StringSheet_RESOURCE, "PANEL_INGAMECASHSHOP_BUYORGIFT_BTN_CANCEL") )	-- 취소
			...
			if workerWrapperLua:isWorking() then
				...
				slot.btn_Restore	:SetFontColor( UI_Color.C_FF626262 )
			else	-- 쉬고 있다.
				slot.progressBg		:SetShow( true )
				...
				slot.btn_Restore	:SetFontColor( UI_Color.C_FFEDE699 )
				...
				if workerWrapperLua:isWorkerRepeatable() and 0 < currentActionPoint then	-- 작업 내역이 있다면, 반복!
					slot.btn_Stop	:SetMonoTone( false )
					slot.btn_Stop	:SetEnable( true )
					slot.btn_Stop	:SetText( "<PAColor0xfff601ff>" .. PAGetString(Defines.StringSheet_RESOURCE, "UI_WORLDMAP_TOWN_MANAGEWORKER_BTN_REACT") .. "<PAOldColor>" )	-- "반복"
					slot.btn_Stop	:addInputEvent( "Mouse_LUp", "HandleClicked_ReDoWork( " .. worker_Index .. " )" )
				end
			end
			...
			if true == hasUpgradeWoker then	-- 승급 중인 일꾼이 있으면, 승급 버튼을 죽인다.
				...
				slot.btn_Upgrade	:SetMonoTone( true )
				slot.btn_Upgrade	:SetEnable( false )
				slot.btn_Upgrade	:SetFontColor( 4286743170 )
				slot.btn_Upgrade	:addInputEvent( "Mouse_LUp", "" )
			end

			slot.bg				:addInputEvent(	"Mouse_On",		"workerManager_ToolTip( true,	" .. worker_Index .. ", " .. limitCount .. " )"		)
			slot.bg				:addInputEvent(	"Mouse_Out",	"workerManager_ToolTip( false )"	)
			...
			if true == workerWrapperLua:getIsAuctionInsert() then	-- 거래소에 등록된 놈이면.
				...
				slot.btn_Stop		:SetMonoTone( true )
				slot.btn_Stop		:SetEnable( false )
				slot.btn_Stop		:SetFontColor( 4286743170 )
...
local workerManager_Update = function()
	...
	-- 패널 사이즈
	Panel_WorkerManager:SetSize( Panel_WorkerManager:GetSizeX(), ((self.slot[0].bg:GetSizeY() + 5) * self.slotMaxCount) + 80 + self._btnFire:GetSizeY() )
