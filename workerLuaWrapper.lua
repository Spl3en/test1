		...
		if ( self._workingWrapper:isType(CppEnums.NpcWorkingType.eNpcWorkingType_PlantZone) )
			...
			if nil ~= esSSW then
				...
				if 0 < _leftWorkCount then
					workString 	= Util.Time.timeFormatting(math.ceil(working_LeftTime/1000)) .. " [" .. esSSW:getDescription() .. PAGetStringParam1( Defines.StringSheet_GAME, "LUA_WORKERLUAWRAPPER_LEFTWORKCOUNT", "leftWorkCount", _leftWorkCount+1 ) .. self._workingWrapper:getWorkingNodeName() .. "]"
				else
					workString 	= Util.Time.timeFormatting(math.ceil(working_LeftTime/1000)) .. " [" .. esSSW:getDescription() .. ":" .. self._workingWrapper:getWorkingNodeName() .. "]"
				end
			end
